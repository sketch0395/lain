...
const CORE_SKILL =
  ...
  "call. This applies especially to save/log/record actions — " +
  "add_threat_intel, save_obsidian_note, remember_fact, log_incident_entry, " +
  "start_incident_report, finish_incident_report, add_playbook, and " +
  "similar — if the user asks you to save/log/add/record something and " +
  "you reply describing it as done (title, category, tags, etc.) without " +
  "having just made that exact tool call in this turn, that is a fabricated " +
  "claim, not a real save, and the user will end up with data loss. When " +
  "the user confirms they want something saved, call the tool in the same " +
  "turn — don't just write prose describing what you would save. If a " +
  "later lookup/search comes back empty for something you previously " +
  "claimed to have saved, that means it was never actually saved — say so " +
  "plainly and offer to save it now for real; never invent an excuse " +
  "(\"the index needs a moment\", \"it takes time to sync\", etc.) to " +
  "explain away a missing result. Also never write a tool call as visible text in your reply — no " +
  ...
  "When reporting the result of a lookup/investigation tool (IP/hash/CVE/" +
  "URL reputation, Shodan, threat intel searches, etc.), be terse: lead " +
  "with the verdict, then a short list of only the fields that actually " +
  "matter for this result — skip narrative preamble, dramatic asides, and " +
  "restating the request back. A long conversation with many lookups adds " +
  "up fast in token cost and hurts your own tool-calling reliability later " +
  "in that same thread, so keep each individual report tight even if your " +
  "personality still comes through.";
...
const CYBER_INTEL_SKILL =
  ...
  "an active or suspected security incident, or explicitly asks to RUN a " +
  "playbook/runbook (e.g. 'run playbook ransomware: Ransomware Response', " +
  "'run the phishing playbook'), call lookup_playbook BEFORE improvising " +
  "your own response steps. If it returns a match, RUN it: call " +
  "start_incident_report right away (see incident report tracking below " +
  "— do this regardless of whether requires_report is true, so findings " +
  "are always captured), then walk the user through the playbook's steps " +
  "in order in your own words, calling log_incident_entry for each step/ " +
  "finding/action as it actually happens — don't just dump the playbook " +
  "verbatim and don't skip ahead. If nothing matches, say so and fall " +
  "back to general incident-response best practice. When the user says " +
  "to close out/wrap up/finish the playbook, call finish_incident_report " +
  "— do this proactively without waiting to be asked if the playbook has " +
  "requires_report: true. You also have add_playbook: use it whenever " +
  "the user asks you to save, write down, or transcribe a NEW playbook/ " +
  "runbook/IR plan — write the content as a clear ordered list of steps, " +
  "since it will be followed literally during a real incident later, not " +
  "just cited like threat intel.\n\n" +
  "IMPORTANT — two completely separate tool families, don't mix them up: " +
  "start_playbook_draft/add_playbook_step/view_playbook_draft/" +
  "finish_playbook_draft/discard_playbook_draft AUTHOR a brand-new " +
  "playbook definition to save in the library. start_incident_report/" +
  "log_incident_entry/view_incident_report_draft/finish_incident_report/" +
  "discard_incident_report RUN an existing playbook (or track any live " +
  "incident) and produce an incident report. If the user is dictating a " +
  "new procedure to be saved for later ('let's build a ransomware " +
  "playbook', 'step one is...'), use the *_playbook_draft tools. If " +
  "they're actually working through an incident — including a test/" +
  "simulated run of an existing playbook via lookup_playbook — use the " +
  "*_incident_report tools instead, never *_playbook_draft.\n\n" +
  "Playbook authoring, step by step: start_playbook_draft/" +
  "add_playbook_step/view_playbook_draft/finish_playbook_draft/" +
  "discard_playbook_draft. Use these instead of a single add_playbook " +
  "call whenever the user wants to talk through a NEW playbook with you " +
  "rather than dictate the whole thing in one message. Call " +
  "start_playbook_draft once at the beginning (category + title), then " +
  "call add_playbook_step once per step as the user describes each one — " +
  "rephrase it into one clear action and briefly confirm back what you " +
  "recorded so they can correct it. Call view_playbook_draft if they " +
  "want to review progress so far. When they say they're done, call " +
  "finish_playbook_draft to save it for real (they'll be asked to " +
  "confirm, same as add_playbook) — or discard_playbook_draft if they " +
  "change their mind partway through. Don't mix approaches: if they've " +
  "started a draft this way, keep adding to it with add_playbook_step " +
  "rather than calling add_playbook separately.\n\n" +
  "Incident report tracking / running a playbook: start_incident_report/" +
  "log_incident_entry/view_incident_report_draft/finish_incident_report/" +
  "discard_incident_report/lookup_incident_report. Call start_incident_report " +
  "the moment you begin actually running/walking-through a playbook found " +
  "via lookup_playbook — do this every time, not only when " +
  "requires_report is true, so findings are never lost. You can also " +
  "start one any time the user asks you to track/document/report on a " +
  "real (or simulated/test) incident, even without a matching playbook. " +
  "Once started, call log_incident_entry as things actually happen — one " +
  "entry per playbook step carried out, finding/piece of evidence " +
  "uncovered, containment/remediation action taken, or lesson learned — " +
  "don't wait and try to reconstruct it all at the end. Use " +
  "view_incident_report_draft if the user wants to review progress. When " +
  "the user says to close out/wrap up/finish the playbook (or the " +
  "incident is resolved), call finish_incident_report to compile " +
  "everything into a full report and save it (they'll be asked to " +
  "confirm, same as add_playbook) — do this proactively without waiting " +
  "to be asked if the playbook required a report — " +
...
const CVE_LOOKUP_SKILL =
  "\n\nYou also have CVE lookup tools: lookup_cve (full details — CVSS " +
  "score/severity/vector, description, CWE, references — for a specific " +
  "CVE ID like CVE-2021-44228) and search_cves (keyword search, e.g. " +
  "'CVEs affecting OpenSSH 9.6' or 'Apache Log4j'). Use these whenever " +
  "the user mentions a CVE ID, asks if something is vulnerable/patched, " +
  "or is researching a vulnerability for a playbook/incident. No API key " +
  "needed. If a CVE is relevant to something the user is dealing with, " +
  "offer to save it with add_threat_intel.";

const IP_REPUTATION_SKILL =
  "\n\nYou also have check_ip_reputation (AbuseIPDB): abuse reports and " +
  "a 0-100 confidence score for an IP — spam, brute-force, scanning, " +
  "botnet activity, etc. Use it when investigating an IP seen in logs, a " +
  "connection, or an alert. This is a different signal from " +
  "shodan_host_lookup (which shows what's exposed ON the IP, not what " +
  "it's been reported doing) — use both together during an " +
  "investigation when Shodan is also available. A score above ~25 with " +
  "multiple reports is worth flagging, above 75 is a strong signal. " +
  "Offer to save notable findings with add_threat_intel.";

const URLHAUS_SKILL =
  "\n\nYou also have check_urlhaus (abuse.ch): checks a URL, host " +
  "(domain/IP), or file hash (MD5/SHA256) against a community-fed " +
  "database of known malware distribution sites and payloads. Give " +
  "exactly one of url/host/hash. This is independent of, and " +
  "complements, check_url_legitimacy (phishing/scam focus) and " +
  "check_file_hash (VirusTotal) — worth checking alongside those, not " +
  "instead of them. found: false just means URLhaus has no record, not " +
  "proof of safety.";

const FILE_REPUTATION_SKILL =
  "\n\nYou also have check_file_hash (VirusTotal): looks up a file's " +
  "MD5/SHA1/SHA256 hash against ~70 antivirus/security engines — " +
  "detection counts, common names, file type, reputation. Use this when " +
  "investigating a suspicious file (pair with hash_file first if you " +
  "only have a path) or a hash mentioned in a report/IOC. Also consider " +
  "check_urlhaus with the same hash for a second, independent signal. " +
  "found: false means VT has no record, not proof the file is safe. " +
  "Offer to save malicious findings with add_threat_intel.";
...
const SKILLS = [
  ...
  {
    key: "cyber_intel",
    text: CYBER_INTEL_SKILL,
    // This one guidance block covers several of cyberIntel.js's
    // sub-categories at once (threat intel logging, playbook authoring/
    // running, web fetch, cyber news) — show it if any of those match.
    category: ["threat_intel", "playbooks", "web_fetch", "cyber_news"],
  },
  { key: "check_updates", text: CHECK_UPDATES_SKILL, category: "system_update" },
  {
    ...
    text: SHODAN_SKILL,
    category: "network",
    configured: (flags) => flags.shodanConfigured,
  },
  { key: "cve_lookup", text: CVE_LOOKUP_SKILL, category: "vuln_lookup" },
  {
    key: "ip_reputation",
    text: IP_REPUTATION_SKILL,
    category: "reputation",
    configured: (flags) => flags.ipReputationConfigured,
  },
  {
    key: "urlhaus",
    text: URLHAUS_SKILL,
    category: "url_safety",
    configured: (flags) => flags.urlhausConfigured,
  },
  {
    key: "file_reputation",
    text: FILE_REPUTATION_SKILL,
    category: "reputation",
    configured: (flags) => flags.fileReputationConfigured,
  },
  {
    key: "obsidian",
    text: OBSIDIAN_SKILL,
    category: "obsidian",
    configured: (flags) => flags.obsidianConfigured,
  },
  {
    key: "laptop_tools",
 ...
 * plus category skills whose keywords/patterns match userText, plus
 * config-gated skills. If nothing matches, only always-on skills are
 * included — matches selectToolDefinitions()'s fallback in
 * lib/tools/registry.js, since there's no point describing a tool that
 * wasn't actually given to the model this turn.
 */
export function buildSkillsPromptAddendum(userText, flags = {}) {
  ...
  for (const skill of SKILLS) {
    ...
    // Both gates apply when both are present (e.g. Shodan only shows up
    // when configured AND the message is network/cyber-intel relevant) —
    // previously "configured" skills ignored relevance entirely and were
    // injected on every single message once an API key was set, which
    // added up fast across unrelated conversations. `category` may be a
    // single name or an array of names (any-of match).
    ...
    const categoryOk =
      ...
      (categories && skillCategories.some((c) => categories.has(c)));
    const configuredOk = !skill.configured || skill.configured(flags);
    if (categoryOk && configuredOk) addendum += skill.text;
  }
  return addendum;
}
