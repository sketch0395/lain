...
/** Updates a remembered fact's content. Returns the updated row, or null if not found. */
export function updateMemory(id, content) {
  const trimmed = String(content || "").trim();
  if (!trimmed) throw new Error("content is required");
  const db = getDb();
  const info = db.prepare(`UPDATE memories SET content = ? WHERE id = ?`).run(trimmed, id);
