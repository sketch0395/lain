...
export async function runToolLoop({ messages, model, conversationId, firstRound }) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (pending) {
      ...
      if (!Array.isArray(toolCalls) || toolCalls.length === 0) {
        return { done: true, content: data.message?.content || "", summaries };
      }
      ...
      if (anyNeedsConfirm) {
        ...
        return { needsConfirmation: true, pendingId, toolCalls };
      }
    }
  ...
  return { done: true, content: "", summaries, hitRoundCap: true };
}
