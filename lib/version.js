...
export async function checkForUpdates() {
  ...
  // Base = our current commit, head = master. GitHub's "ahead_by" is how
  // far *head* (master) is ahead of *base* (us) — i.e. exactly how many
  // commits we're behind. ("behind_by" would only be nonzero if our commit
  // had unpushed commits master doesn't have — shouldn't normally happen.)
  return {
    repo: GITHUB_REPO,
    branch: GITHUB_BRANCH,
    currentCommit: CURRENT_COMMIT,
    upToDate: data.status === "identical",
    commitsBehind: data.ahead_by || 0,
