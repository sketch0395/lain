...
export const REMINDER_TOOL_DEFINITIONS = [
  ...
  // shodan_host_lookup/shodan_search/shodan_dns_lookup/shodan_account_info
  // definitions live in the shared assistant-tools submodule
  // (tools-agent/shared/tools/shodan.js). Only offered when SHODAN_API_KEY
  // is actually set, since without it every call would just fail.
  ...(shodanConfigured()
...
const TOOL_CATEGORIES = {
  // Laptop/system forensics — files, processes, logs, packet captures.
  system_diagnostics: ["forensics"],
  hash_file: ["forensics"],
  file_metadata: ["forensics"],
  extract_strings: ["forensics"],
  list_processes: ["forensics"],
  network_connections: ["forensics"],
  search_logs: ["forensics"],
  recent_file_activity: ["forensics"],
  login_history: ["forensics"],
  analyze_pcap: ["forensics"],
  capture_packets: ["forensics"],
  find_files: ["forensics"],
  search_files: ["forensics"],
  read_file: ["forensics"],
  list_directory: ["forensics"],
  summarize_directory: ["forensics"],
  // Network diagnostics and lookups.
  ping_host: ["network"],
  dns_lookup: ["network"],
  traceroute_host: ["network"],
  whois_lookup: ["network"],
  port_scan: ["network"],
  check_port: ["network"],
  lan_device_scan: ["network"],
  speed_test: ["network"],
  shodan_host_lookup: ["network"],
  shodan_search: ["network"],
  shodan_dns_lookup: ["network"],
  shodan_account_info: ["network"],
  // Omarchy desktop control (themes, commands, backgrounds).
  omarchy_status: ["omarchy"],
  list_omarchy_themes: ["omarchy"],
  set_omarchy_theme: ["omarchy"],
  list_omarchy_commands: ["omarchy"],
  omarchy_command: ["omarchy"],
  omarchy_command_background: ["omarchy"],
  create_omarchy_theme: ["omarchy"],
  extract_image_colors: ["omarchy"],
  create_omarchy_theme_from_image: ["omarchy"],
  // Self-update / version checks.
  update_asuna: ["system_update"],
  update_lain: ["system_update"],
  check_for_updates: ["system_update"],
  // Threat intel / cyber news / web fetch.
  lookup_threat_intel: ["cyber_intel"],
  add_threat_intel: ["cyber_intel"],
  fetch_web_page: ["cyber_intel"],
  get_cyber_news: ["cyber_intel"],
  add_cyber_news_source: ["cyber_intel"],
  list_cyber_news_sources: ["cyber_intel"],
  remove_cyber_news_source: ["cyber_intel"],
  set_cyber_news_watch_terms: ["cyber_intel"],
};
  ...
  switch (name) {
    ...
    default:
      return (
        ...
        shodanTool.describeToolCall(name, args) ||
        `run ${name}`
      );
  }
}
...
export async function callTool(name, rawArgs) {
  ...
  switch (name) {
    ...
    case "ping_host": {
      ...
      url = `${TOOLS_URL}/ping?${params}`;
      break;
    }
    case "dns_lookup": {
      const params = new URLSearchParams({ host: args.host || "" });
      url = `${TOOLS_URL}/dns-lookup?${params}`;
      break;
    }
    case "traceroute_host": {
      const params = new URLSearchParams({ host: args.host || "" });
      if (args.max_hops) params.set("maxHops", String(args.max_hops));
      url = `${TOOLS_URL}/traceroute?${params}`;
      break;
    }
    case "whois_lookup": {
      const params = new URLSearchParams({ query: args.query || "" });
      url = `${TOOLS_URL}/whois?${params}`;
      break;
    }
    case "port_scan": {
      const params = new URLSearchParams({ host: args.host || "" });
      if (args.ports) params.set("ports", args.ports);
      url = `${TOOLS_URL}/port-scan?${params}`;
      break;
    }
    case "check_port": {
      const params = new URLSearchParams({ host: args.host || "", port: String(args.port || "") });
      url = `${TOOLS_URL}/check-port?${params}`;
      break;
    }
    case "lan_device_scan": {
      const params = new URLSearchParams();
      if (args.subnet) params.set("subnet", args.subnet);
      url = `${TOOLS_URL}/lan-scan?${params}`;
      break;
    }
    case "speed_test":
      url = `${TOOLS_URL}/speed-test`;
      break;
    case "capture_packets": {
    ...
    case "list_omarchy_commands":
      url = `${TOOLS_URL}/omarchy/commands`;
      break;
    case "omarchy_command": {
      const res = await fetch(`${TOOLS_URL}/omarchy/command`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
    ...
    case "omarchy_command_background": {
      const res = await fetch(`${TOOLS_URL}/omarchy/command/background`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({ argv: args.argv }),
        signal: AbortSignal.timeout(15000),
    ...
    case "create_omarchy_theme": {
      const res = await fetch(`${TOOLS_URL}/omarchy/theme/create`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({
          name: args.name,
    ...
    case "extract_image_colors": {
      const res = await fetch(`${TOOLS_URL}/omarchy/image/colors`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({ path: args.path, count: args.count }),
    ...
    case "create_omarchy_theme_from_image": {
      const res = await fetch(`${TOOLS_URL}/omarchy/theme/from-image`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
        body: JSON.stringify({
          name: args.name,
  ...
  // whois/port-scan spend real wall-clock time waiting on the network —
  // give all of these more room than the default 10s budget used by quick
  // lookups.
