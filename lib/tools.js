...
export const LAPTOP_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      name: "create_note",
      description:
        "Create, append to, or replace a markdown (.md) note file in the " +
        "user's Documents folder on their laptop. Use for saving " +
        "summaries, lists, or any content the user asks to have written " +
        "down/saved as a file (as opposed to a durable memory fact). " +
        "Matching is by exact title against existing notes.",
      parameters: {
        type: "object",
        properties: {
          title: {
            ...
            description:
              ...
              "top-level heading in the file. Also used to find an " +
              "existing note for append/replace mode.",
          },
          content: {
            type: "string",
            description: "The note's body content, in markdown.",
          },
          mode: {
            type: "string",
            enum: ["create", "append", "replace"],
            description:
              "'create' (default) always makes a brand-new, distinct note " +
              "— use when the user wants a new note even if a similarly-" +
              "titled one exists. 'append' adds this content to the end " +
              "of the existing note with this exact title (e.g. 'add X " +
              "to my grocery list'). 'replace' overwrites the entire " +
              "content of the existing note with this exact title.",
          },
        },
        required: ["title"],
      },
