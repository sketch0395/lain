...
export const LAPTOP_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Capture live network traffic on the user's laptop for a short, bounded " +
        "time and report protocol counts, top talkers, and sample packet lines — " +
        "e.g. 'capture some traffic and tell me what's talking to the internet " +
        "right now'. Requires the tools agent's tcpdump to have packet-capture " +
      ...
      parameters: {
        ...
        properties: {
          duration: {
            type: "number",
            description: "How many seconds to capture for (default 10, max 60).",
          },
          limit: {
            type: "number",
            description:
              "Stop early once this many packets are captured (default 100, max 2000).",
          },
          interface: {
            type: "string",
            description:
          ...
          save_path: {
            type: "string",
            description:
              "Optional path to also save the raw capture as a .pcap file for later " +
              "analysis (e.g. with analyze_pcap), within directories the user has " +
              "allowed. Any missing folders in the path are created " +
              "automatically, e.g. '~/Documents/pcaps/capture1.pcap' creates the " +
              "pcaps folder if it doesn't exist yet. '.pcap' is appended if the " +
              "name doesn't already end in a recognized pcap extension.",
          },
        },
        required: [],
      },
...
export async function callTool(name, rawArgs) {
  ...
  switch (name) {
    ...
    case "list_directory": {
      ...
      if (args.root) params.set("root", args.root);
      if (args.limit) params.set("limit", String(args.limit));
      url = `${TOOLS_URL}/list?${params}`;
      break;
    }
    case "summarize_directory": {
      const params = new URLSearchParams({ root: args.root || "" });
      if (args.limit) params.set("limit", String(args.limit));
      url = `${TOOLS_URL}/read-dir?${params}`;
      break;
    }
    case "hash_file": {
      const params = new URLSearchParams({ path: args.path || "" });
    ...
    case "capture_packets": {
      // Give the agent enough time for `duration` (clamped server-side to
      // 60s) plus a buffer for process startup/summarization.
      const durationMs = Math.min(Math.max(Number(args.duration) || 10, 1), 60) * 1000;
